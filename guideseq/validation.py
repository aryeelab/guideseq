...
def checkIfBinary(filepath):
    ...
    if not is_binary_string(open(filepath, 'rb').read(1024)):
        logger.error('{0} is not a valid binary'.format(filepath))
        sys.exit()
...
def checkIfValidDemultiplexed(demultiplexed):
    # Check if read1, read2, index1, and index2 exist
    fields = ['read1', 'read2', 'index1', 'index2']

    if set(fields) != set(demultiplexed.keys()):
        logger.error('Demultiplexed field must contain references to "read1", "read2", "index1", "index2"')
        sys.exit()

    invalid_file = False
    for field in fields:
        if not os.path.isfile(demultiplexed[field]):
            logger.error('"read1" field does not reference a valid file')
            invalid_file = True
...
def checkIfValidSamples(samples):
    ...
    for sample in samples:
        if 'barcode1' not in sample or 'barcode2' not in sample:
...
def validateManifest(manifest_data):
    ...
    fields = ['bwa', 'bedtools', 'reference_genome', 'output_folder', 'samples', 'demultiplexed']
    ...
    checkIfFolder(manifest_data['output_folder'])
    checkIfValidDemultiplexed(manifest_data['demultiplexed'])
